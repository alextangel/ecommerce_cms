...
<html>
	...
	<body>
		...
		<div class="container">
			...
			$numberOfRows = mysql_num_rows(mysql_query('SELECT imagesrc FROM msproduct where producttypeid = 1'));
			$totalPages = ceil($numberOfRows / $resultsPerPage);


			if(isset($_POST['searchBy']) && ($_POST['searchBy'] == 'name') && isset($_POST['search']))
			{
			        $q = sprintf("SELECT imagesrc, productname, price, isnew, description, productid FROM msproduct WHERE productname like '%%%s%%' AND producttypeid = 1",
			                       mysql_real_escape_string($_POST["search"]));
			        $query = mysql_query($q);
			}
			else if(isset($_POST['searchBy']) && ($_POST['searchBy'] == 'desc') && isset($_POST['search']))
			{
			        $q = sprintf("SELECT imagesrc, productname, price, isnew, description, productid FROM msproduct WHERE description like '%%%s%%' AND producttypeid = 1",
			                       mysql_real_escape_string($_POST["search"]));
			        $query = mysql_query($q);
			}
			else if(isset($_POST['searchBy']) && ($_POST['searchBy'] == 'price') && isset($_POST['search']))
			{
			        $q = sprintf("SELECT imagesrc, productname, price, isnew, description, productid FROM msproduct WHERE price = '%s' AND producttypeid = 1",
			                       mysql_real_escape_string($_POST["search"]));
			        $query = mysql_query($q);
			}			
			else
			{
				$query = mysql_query("SELECT imagesrc, productname, price, isnew, description, productid FROM msproduct where producttypeid = 1 LIMIT $startResults, $resultsPerPage");
			}
